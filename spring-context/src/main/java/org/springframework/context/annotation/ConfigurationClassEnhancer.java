...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		//如果该类的父接口是 EnhancedConfiguration.class  代表已经进行过cglib代理
		...
		/**
		 * 进行 cglib代理 先设置 cglib 的spring生成器，再通过生成器的 creatClass 得到代理对象
		 */
	...
	private Enhancer newEnhancer(Class<?> superclass, @Nullable ClassLoader classLoader) {
		...
		//增强接口  表示一个类被代理了 同时实现了setBeanFactory 这个方法
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
		/**
		 * 重要！！！
		 * 添加了一个生成策略， 就是为我们代理的类添加了一个属性 =  BeanFactory  $$BeanFactory = null 暂时没有赋值
		 * 以后肯定会赋值
		 * 好处就是我们不用添加任何一个方法，只需要通过这个属性中的方法就能实现 getBean() 。。。等等
		 */
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));
		/**
		 * 重要！！！
		 * 添加回调过滤器，也就是对每个方法进行增强。
		 *    1.BeanMethodInterceptor(),
		 *     	  判断 方法是否有@bean 注解如果有就进行了增强 ，并且会对这些方法进行判断
		 *     	  如果beanFactory中单例池已经存在了该对象，则不会创建
		 *
		 *     	  对这些方法内部的方法也要考虑是否cglib代理
		 *
		 *    2.BeanFactoryAwareMethodInterceptor(),
		 *     	  判断  $$beanFactory 这个属性是否有实例，有就直接返回，没有去创建
		 */
	...
	private static class BeanFactoryAwareGeneratorStrategy extends DefaultGeneratorStrategy {
		...
		protected ClassGenerator transform(ClassGenerator cg) throws Exception {
			//添加了一个属性  BeanFactory $$beanFactory = null;
	...
	BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//然后通过方法获取 beanName，默认为方法名，也可以通过 @Bean 注解指定
			...
			/**
			 *	判断当前执行的方法是否是第一次执行，spring中判断是否是第一次执行很牛逼！！
			 *
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			try {
				...
				//直接把该方法对应的beanDefinition 实例化后 放入单例池中
				...
				if (alreadyInCreation) {
					//把该bean添加进beanFactory中的当前创建的map集合中 并设置为true

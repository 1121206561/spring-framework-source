...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			//根据basePackage加载包下所有java文件，并扫描出所有bean组件，并且转换成beanDefinition
			...
			for (BeanDefinition candidate : candidates) {
				...
				/**
				 *  此时bd自己的注解配置放在 bd中的metadata中
				 *  从metadata中解析到candidate结构中，主要是处理Lazy, primary DependsOn, Role ,Description这五个注解
				 */
				...
				if (checkCandidate(beanName, candidate)) {
					 ...
					 * 此时仅仅只是把扫描出来的类 创建成beanDefinition并发到map集合中 ， 并没有直接放到单例池中
 					 */

...
public abstract class AopProxyUtils {
	...
	static Class<?>[] completeProxiedInterfaces(AdvisedSupport advised, boolean decoratingProxy) {
		 ...
		 * 如果目标类它自身就是个接口，将其加到目标Interface；
		 * 如果目标类本身已经是一个代理类（根据Proxy.isProxyClass判断），是JDK动态代理创建的代理对象，
		 * 那么将其所有的接口都加到目标Interface；
		 * 重新获取目标接口
		 */
		...
		/**
		 * addSpringProxy、addAdvised用于判断目标类是否是特殊接口：SpringProxy、Advised。
		 * 一般情况下两个boolean都是true；
		 */
		if (addSpringProxy) {
		...
		//返回一个数组集合 包含 标代理类、SpringProxy、Advised三个接口

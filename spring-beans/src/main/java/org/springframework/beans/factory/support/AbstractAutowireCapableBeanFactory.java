...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		/**
		 * 遍历容器，为当前bean添加执行所有存在于 BeanFactory 的 BeanPostProcessor 中的后置处理器
		 * 执行过的不会在执行了
		 */
		for (BeanPostProcessor beanProcessor : getBeanPostProcessors()) {
			/**
			 * 执行后置处理器的 postProcessBeforeInitialization方法
			 * 执行顺序 1.spring内置的
			 * 			2.用户自己实现接口添加的beanPostProcessor
			 */
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor beanProcessor : getBeanPostProcessors()) {
			 ...
			 *
			 * aop的后置处理器就是在这里完成代理
			 * 为什么jdk动态代理要求必须使用接口 因为通过jdk代理生成的对象默认   继承 proxy父类
			 * 他不能继承代理之前的对象  只能通过实现 之前对象的接口才可以完成代理
			 */
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			//处理 lookup-method 和 replace-method 配置，spring将这两个类统称为准备方法
		...
		try {
			...
			/**
			 * 这个方法是判断当前正在创建的类是否实现了InstantiationAwareBeanPostProcessor这个后置处理器接口
			 * 如果实现该接口，那么当前类就会在这个方法，通过
			 * 然后直接返回该bean
			 */
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			 ...
			 *  通过反射创建一个对象  只是new出来了一个对象并没有进行属性的注入
			 */
		...
		try {
			...
			/**
			 * 对bean属性进行依赖注入 即自动注入 ,就会产生循环依赖的问题
			 * 太难了！！！！
			 */
			 ...
			 *  为 bean对象执行 beanPostProcessor 后置处理器
			 *
			 *   完成剩下几乎所有的步骤  执行aware、初始化回调方法、aop ....等等
			 *
			 *   但是 注入 SingletonObjects....并没有完成
 			 */
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
					SmartInstantiationAwareBeanPostProcessor ibp = (SmartInstantiationAwareBeanPostProcessor) bp;
					/**
					 * 其中有个重要的后置处理器  AbstractAutoProxyCreator 就是进行AOP，也证明循环依赖解决了AOP问题
					 */
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		/**
		 * 解析设置的 FactoryMethod 方法
		 * 如果该类设置了 FactoryMethod 的方法那么也不会走生命周期，直接执行 FactoryMethod指定的方法
		 ...
		 *这里的resolved和mbd.constructorArgumentsResolved 将会在bean 第一次实例化的过程中被设置，后面来证明
		 */
		 ...
		 *   4.如果是多个，且有一个加了@AutoWried=true 就返回这个加@AutoWried的这个构造器
		 *   5.如果是多个，且有多个加了@AutoWried=true的，这个spring会异常，会报错，
		 *   6.如果是多个，且有多个加了@AutoWried=false的—>会返回多个构造方法—>会再次进行推断，
		 *
		 *
		 *   如果是多个，这个时候spring迷茫，不知道该用哪个，就把所有的构造函数都传递给真正实例化对象的时候，
		 *   那个时候会再次进行解析找出最优的
		 * 		spring怎么找出最优的，
		...
		if (ctors != null ||
			 ...
			 * 	   5.
			 * 	这个方法很难
			 * 	大部分情况推断出来的构造方法都是null，或者只有一个有参的构造方法，
			 *
			 */
		...
		/**
		 *	如果没有推断出构造方法 则会使用默认的构造方法 通过反射创建对象
		 */
		return instantiateBean(beanName, mbd);
	}
	...
	protected Constructor<?>[] determineConstructorsFromBeanPostProcessors(@Nullable Class<?> beanClass, String beanName)
		 ...
		 * determineCandidateConstructors(java.lang.Class, java.lang.String)
		 */
		if (beanClass != null && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {

				/**
				 * 此时有两个后置处理器，实现了SmartInstantiationAwareBeanPostProcessor接口
				 * 第一次返回null，如果不为null证明不需要推断
				 * 第二次真正去判断构造方法
				 */
	...
	protected void autowireByName(
		...
		for (String propertyName : propertyNames) {
			/**
			 * 容器中存在这个名称的BeanDefinition或者单例Bean
			 */
	...
	protected void autowireByType(
		...
		/**
		 * 获取符合参与条件的属性(ByName中也用到了)，
		 * 先获取有get/set/is方法的所有属性，继承下来的也会得到
		 * 然后进行过滤，移除掉不符合规则的
		 */
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		/**
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					/**
					 * 获得属性的setter方法的描述(就是setter方法的一些信息)
					 */
					MethodParameter methodParam = BeanUtils.getWriteMethodParameter(pd);
					// Do not allow eager init for type matching in case of a prioritized post-processor.
					/**
					 * 如果Bean实现了PriorityOrdered接口，则不进行类型匹配，这针对后置处理器
					 */
					...
					/**
					 * 根据setter方法的入参类型确定依赖项，根据类型在容器中查找类并创建实例并返回
					 * 同时放到了autowiredBeanNames集合中
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		/**
		 * 获取BeanDefinition中手动设置的属性，
		 */
		PropertyValues pvs = mbd.getPropertyValues();
		/**
		 * 获取Class中所有的setter或getter对应的属性
		 * 属性名称就是根据setter或getter方法的方法名去掉set或get前缀
		 * 如果是boolean类型的，那么就会找到  isXX 方法去掉前缀
		 *
		 * WriteMethod = set方法
		 * readMethod = get方法
		 */
		...
		for (PropertyDescriptor pd : pds) {
			 ...
			 * 	1、属性存在setter方法
			 * 	2、不在一些排除的规则中
			 * 	3、不在BeanDefinition中显示设置的属性列表中
			 * 	4、不是规定的简单类型的属性  Class类型被排除
			 */
	...
	/**
	 * 执行所有aware方法的前提是当前类实现了这些aware接口
	 */
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (mbd == null || !mbd.isSynthetic()) {
			/**
			 * 因为所有后置处理器都实现了 beanPostProcessor 接口
			 * 这个接口有两个方法
			 * 		1. postProcessBeforeInitialization
			 * 		2. postProcessAfterInitialization
			 *  但是并不是所有的后置处理器只在这个时候执行，因为部分后置处理器有自己独有的方法，在实例化的过程中这些方法
			 *  就已经被执行力。
			 *  这里只是执行公有的两个方法，即父接口的两个方法
			 *
			 * 为当前bean执行 BeanFactory中的 BeanPostProcessor 集合中的所有 后置处理器的 postProcessBeforeInitialization 方法
			 *
		...
		if (mbd == null || !mbd.isSynthetic()) {
			/**
			 * 为当前bean执行 BeanFactory中的 BeanPostProcessor 集合中的所有 处理器的 postProcessAfterInitialization 方法
			 */
	...
	private void invokeAwareMethods(final String beanName, final Object bean) {
		...
		if (bean instanceof Aware) {
			if (bean instanceof BeanNameAware) {
				/**
				 * 执行方法给bean的属性beanName设置对应的beanName
				 */
			...
			if (bean instanceof BeanClassLoaderAware) {
				...
				if (bcl != null) {
					/**
					 * 执行方法给bean的属性beanClassLoaderAware设置beanFactory的类加载器
					 */
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		if (mbd != null && bean.getClass() != NullBean.class) {
			...
			//判断是否指定了init-method方法，如果指定了init-method方法，则调用init-method方法
			if (StringUtils.hasLength(initMethodName) &&
					!(isInitializingBean && "afterPropertiesSet".equals(initMethodName)) &&
					!mbd.isExternallyManagedInitMethod(initMethodName)) {
				/**
				 * 调用方法，结论是通过反射调用，里面看不懂！！
				 */

...
public class MapperAnnotationBuilder {
	...
	void parseStatement(Method method) {
		...
		/**
		 *  拿到解析sql语言的驱动，如果在方法上加了@lang注解，那么就设置为该注解的内容
		 *  没设置就使用默认的XMLLanguageDriver驱动器
		 */
		...
		/**
		 * 如果存在sql语句继续执行，否则直接结束本次方法
		 */
		if (sqlSource != null) {
			/**
			 * 拿到@Options注解
			 * @Options注解能够设置缓存时间，能够为对象生成自增的主键值
			 */
			...
			//拿到sql类型  select\update\delete\insert
			SqlCommandType sqlCommandType = getSqlCommandType(method);

			/**
			 * 判断是否是select类型 ，如果是则不需要进行缓存清除，并且使用缓存保存结果
			 * 这也证明了mybatis中为什么只有select语句才有缓存，并且执行除了select类型的sql语句
			 * 都会清空缓存
			 */
	...
	private SqlSource getSqlSourceFromAnnotations(Method method, Class<?> parameterType, LanguageDriver languageDriver) {
		try {
			/**
			 * 拿到方法上的 @select/update/delete/insert 注解
			 */
			Class<? extends Annotation> sqlAnnotationType = getSqlAnnotationType(method);
			/**
			 * 拿到法上的 @select/update/delete/insert + provider 的注解
			 */
			...
			if (sqlAnnotationType != null) {
				...
				/**
				 * 拿到具体的sql语句，因为value是一个数组，所以用数组保存
				 * 为什么是数组，防止sql语句过长，
				 */
				final String[] strings = (String[]) sqlAnnotation.getClass().getMethod("value").invoke(sqlAnnotation);
				/**
				 *  拼接每个String拿到完整的sql语句，然后封装成sqlSource对象返回给用户，主要就是保存了sql语句
				 */
